...
(async () => {
  try {
    ...
    ws.on('message', (data) => {
      ...
      if (m.id === 2 && m.result) {
        console.log('Layout metrics:', JSON.stringify(m.result, null, 2));
        // Wait a bit for stability
        setTimeout(() => {
          // Try screenshot with clip from layout
          const {contentSize} = m.result;
          const clip = {
            x: 0,
            y: 0,
          ...
          send('Page.captureScreenshot', {format:'png', clip, fromSurface:true});
        }, 500);
      }
      if (m.id === 3 && m.result) {
