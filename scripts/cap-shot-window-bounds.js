  ...
  try {
    ...
    ws.on('message', (data) => {
      ...
      if (m.id === 1 && m.result) {
        ...
        send('Page.getWindowBounds', {});
      }
      if (m.id === 2 && m.result) {
        console.log('Window bounds:', JSON.stringify(m.result, null, 2));
        const {bounds} = m.result;
        // Wait a bit for stability
        setTimeout(() => {
          // Capture screenshot with clip set to the window bounds
